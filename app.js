...
function obtenerVentanaImpresion() {
  ...
  // Asegurarse de que la ventana esté completamente cargada
  if (ventana.document.getElementById('contenido') === null) {
    ventana.onload = function() {
      ventana.document.getElementById('contenido').innerHTML = '';
    };
  }
...
function procesarPago() {
  ...
  if (mesaSeleccionada.startsWith('DOM-')) {
    ...
    // Agregar información del cliente incluso si es mesa normal
    infoAdicional = `
      <div class="border-top">
        <div class="mb-1">Cliente: ${pedido.cliente}</div>
        <div class="mb-1">Tel: ${pedido.telefono || 'No especificado'}</div>
    ...
    <html>
      ...
      <body>
        ...
        <div class="header text-center">
          ...
          <div class="mb-1">${new Date(factura.fecha).toLocaleString()}</div>
...
function generarReciboPreliminar() {
  ...
  // Crear nueva ventana
  const ventanaPrevia = window.open('', '_blank', 'width=400,height=600,scrollbars=yes');
  if (!ventanaPrevia) {
    alert('No se pudo abrir la ventana de impresión. Por favor, verifique que los bloqueadores de ventanas emergentes estén desactivados.');
    return;
  }
